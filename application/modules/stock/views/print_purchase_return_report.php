...
<div id="page-inner">
	<div class="row">
		<div class="col-md-12">
			...
			<h2>
				<?php echo $this->lang->line('purchase') . ' ' .$this->lang->line('return') . ' ' . $this->lang->line('report');?>
			</h2>
			<h4>
			...
			<div class="panel-body">
				<div class="table-responsive">
					<table class="table table-striped table-bordered table-hover" id="purchase_table">
						...
						<tbody>
						<?php
							foreach ($purchase_return_totals as $purchase_return_total) {
								$found = FALSE;
								$show_total = FALSE;
								foreach ($purchase_returns as $purchase_return) {
									if ($purchase_return_total['bill_no'] == $purchase_return['bill_no'] && $purchase_return_total['return_date'] == $purchase_return['return_date'] ) {
										$date = $purchase_return['return_date'];
										$bill_no = $purchase_return['bill_no'];
										$supplier_name = $purchase_return['supplier_name'];
										$item_name = $purchase_return['item_name'];
										$qnt = $purchase_return['quantity'];
										$cost = $purchase_return['price'];
										$amount = ($purchase_return['price']);
										$found = TRUE;
										$show_total = TRUE;
									}
									if ($found) {
							...
							?>
								<tr>
									...
									<td style="text-align: right;"><strong><?php echo currency_format($purchase_return_total['total']); if($currency_postfix) echo $currency_postfix; ?></strong></td>
								</tr>
								<?php 
								$show_total = FALSE;
								} ?>
