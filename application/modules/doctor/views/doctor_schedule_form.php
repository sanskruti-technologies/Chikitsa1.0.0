...
<?php 	
	$days_of_week = array("Sunday" => 'sunday',
	                      "Monday" => 'monday',
						  "Tuesday" => 'tuesday',
						  "Wednesday" => 'wednesday',
						  "Thursday" => 'thursday',
							...
							?>
								<select name="doctor_id" class="form-control"><option></option>
									<?php foreach ($doctors as $doctor) { ?>
										<option value="<?php echo $doctor['doctor_id'] ?>" <?php if($doctor['doctor_id'] == $schedule_doctor_id){echo "selected";}?>><?= $doctor['title'] . ' ' .$doctor['first_name'] . ' ' . $doctor['middle_name']. ' ' . $doctor['last_name']; ?></option>
									<?php } ?>
								</select>
				...
				<div id='day_group'>
					...
					<div class="form-group" >
						<?php foreach($days_of_week as $key => $value){ ?>
							<label class="checkbox-inline" >
								<input name="day[]" type="checkbox" <?php if(strpos($schedule_day,$key) === false){}else{echo "checked";} ?> value="<?=$key;?>"><?=$this->lang->line($value);?>
							</label>
						<?php } ?>
